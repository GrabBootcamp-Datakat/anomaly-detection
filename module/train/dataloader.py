...
def load_Spark(log_file, label_file=None, window='session', train_ratio=0.5, split_type='sequential', save_csv=False, window_size=0):
    ...
    if log_file.endswith('.npz'):
        ...
        for idx, row in struct_log.iterrows():
            ...
            if container_id not in data_dict:
                data_dict[container_id] = []
            data_dict[container_id].append(row['EventId'])

        # Convert to DataFrame with distinct Container_IDs and their Event sequences
        data_df = pd.DataFrame(list(data_dict.items()), columns=['Container_ID', 'EventSequence'])
        # for idx, row in struct_log.iterrows():
        ...
        if label_file:
            ...
            label_data = label_data.set_index('Container_ID')
            label_dict = label_data['Label'].to_dict()
            data_df['Label'] = data_df['Container_ID'].apply(lambda x: 1 if label_dict[x] == 'abnormal' else 0)
...
def slice_spark(x, y, window_size):
    results_data = []

...
class LogParser:
    ...
    def outputResult(self, logClustL):
        ...
        # Step 2: Generate unique EventIds like E1, E2, ... for each unique template
        unique_templates = self.df_log["EventTemplate"].unique()
        template_to_id = {template: f"E{i+1}" for i, template in enumerate(unique_templates)}

        self.df_log["EventId"] = self.df_log["EventTemplate"].map(template_to_id)
        ...
        # Step 3: Save structured logs
        self.df_log.to_csv(
            os.path.join(self.savePath, self.logName + "_structured.csv"),
            index=False
        )

        # Step 4: Build and save template table
        df_event = pd.DataFrame({
            "EventTemplate": unique_templates,
        })
        df_event["EventId"] = df_event["EventTemplate"].map(template_to_id)
        # df_event = pd.DataFrame(
        ...
        # template_to_id = {template: f"E{i+1}" for i, template in enumerate(df_event["EventTemplate"])}
